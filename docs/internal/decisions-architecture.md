...
divergencia de layout), y el corpus queda en 5 casos pendientes.
...
**El contrato entre partials, fijado al fusionar mentalmente con UN6/UN7:** el handle del
`zb_view*` se pide por `NativeViewHandle()` —el hook `partial void NativeView(ref IntPtr)`
que declara `Pad.cs` (UN6) e implementa `Host.cs` (UN7)—, y la propiedad del input por
`InputOwner.Register/Owns/Claim` (UN6). Las cuatro ramas de Wave B nacieron de un chasis sin
ninguna de las dos cosas; los nombres se alinearon leyendo los worktrees hermanos, y los
`.meta` de las carpetas compartidas (`Runtime/Input`, `Tests/PlayMode`) llevan el mismo
GUID que en UN6 para que la fusión no los pelee.
