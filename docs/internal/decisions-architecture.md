...
él **`gamepad-nav` sale de la skip-list comparando byte a byte a la primera**. El corpus
queda en 2 casos pendientes, los dos de G12.
...
prueba de que no es un segundo modelo de input. `scroll_focused_by` lee el foco
**realizado**; la rama del foco lógico —una fila que un `Repeat` no ha materializado— la
cablea G12 (ZAB-145), y queda anotada en el sitio.
