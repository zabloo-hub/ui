...
/**
 * The corpus index and the perf index, parsed once for the life of the process.
 *
 * Held by value because a `JsonRef` is a cursor into its document: every ref
 * handed out below would dangle the moment either was rebuilt.
 */
struct Indexes {
  JsonParse cases;
  JsonParse perf;
...
const Indexes &indexes() {
  static const Indexes parsed = [] {
    Indexes out;
    out.cases = JsonDoc::parse(corpus_file("cases.json"));
    out.perf = JsonDoc::parse(corpus_file("perf/scenes.json"));
    return out;
  }();
  return parsed;
}

JsonRef perf_index() { return indexes().perf.doc.root(); }
...
JsonRef corpus_cases() { return indexes().cases.doc.root(); }
